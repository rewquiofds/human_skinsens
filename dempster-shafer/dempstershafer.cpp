...
Evidence Evidence::operator&(Evidence& other) {
    ...
    for (list<FocalSet>::iterator k = focal_sets.begin(); k != focal_sets.end(); k++) {
        FocalSet combined;
        combined.mass = 0;
        combined.items = k->items;
        for (list<FocalSet>::iterator i = focal_sets.begin(); i != focal_sets.end(); i++) {
            for (list<FocalSet>::iterator j = other.focal_sets.begin(); j != other.focal_sets.end(); j++) {
                if ((i->items & j->items) == k->items) {
    ...
    //double c = 0;  // Yager
    double c = 1 / (1 - conflict);  // Dempster
    //double c = 1 / (1 - conflict - set_buffer.back().mass);  // Inagaki

    for (list<FocalSet>::iterator i = set_buffer.begin(); i != set_buffer.end(); i++) {
        if (std::next(i) == set_buffer.end()) {
            i->mass = (1 + c * conflict) * (set_buffer.back().mass + conflict) - c * conflict;
        }
        else {
            i->mass *= (1 + c * conflict);
        }
...
/**************************** 这部分我加的 ****************************/
double Evidence::conflictTwo(Evidence& e1, Evidence& e2) {
    double conflict = 0.0;

    // iterate pair-wise over all focal-sets of both evidences
    for (list<FocalSet>::iterator i = e1.focal_sets.begin(); i != e1.focal_sets.end(); i++) {
        for (list<FocalSet>::iterator j = e2.focal_sets.begin(); j != e2.focal_sets.end(); j++) {
            if ((i->items & j->items).none()) {
                // and add the combined mass to the conflict if the intersection is empty
                conflict += i->mass * j->mass;
            }
...
double Evidence::conflictThree(Evidence& e1, Evidence& e2, Evidence& e3) {
    double conflict = 0.0;

    // iterate pair-wise over all focal-sets of both evidences
    for (list<FocalSet>::iterator i = e1.focal_sets.begin(); i != e1.focal_sets.end(); i++) {
        for (list<FocalSet>::iterator j = e2.focal_sets.begin(); j != e2.focal_sets.end(); j++) {
            for (list<FocalSet>::iterator k = e3.focal_sets.begin(); k != e3.focal_sets.end(); k++) {
                if ((i->items & j->items & k->items).none()) {
                    // and add the combined mass to the conflict if the intersection is empty
                    conflict += i->mass * j->mass * k->mass;
                }
            }
        }
    }
...
double Evidence::conflictFour(Evidence& e1, Evidence& e2, Evidence& e3, Evidence& e4) {
    double conflict = 0.0;
    ...
    for (list<FocalSet>::iterator i = e1.focal_sets.begin(); i != e1.focal_sets.end(); i++) {
        for (list<FocalSet>::iterator j = e2.focal_sets.begin(); j != e2.focal_sets.end(); j++) {
            for (list<FocalSet>::iterator k = e3.focal_sets.begin(); k != e3.focal_sets.end(); k++) {
                for (list<FocalSet>::iterator m = e4.focal_sets.begin(); m != e4.focal_sets.end(); m++) {
                    if ((i->items & j->items & k->items & m->items).none()) {
                        // and add the combined mass to the conflict if the intersection is empty
                        conflict += i->mass * j->mass * k->mass * m->mass;
                    }
                }
            }
        }
...
double Evidence::conflictFive(Evidence& e1, Evidence& e2, Evidence& e3, Evidence& e4, Evidence& e5) {
    double conflict = 0.0;
    ...
    for (list<FocalSet>::iterator i = e1.focal_sets.begin(); i != e1.focal_sets.end(); i++) {
        for (list<FocalSet>::iterator j = e2.focal_sets.begin(); j != e2.focal_sets.end(); j++) {
            for (list<FocalSet>::iterator k = e3.focal_sets.begin(); k != e3.focal_sets.end(); k++) {
                for (list<FocalSet>::iterator m = e4.focal_sets.begin(); m != e4.focal_sets.end(); m++) {
                    for (list<FocalSet>::iterator n = e5.focal_sets.begin(); n != e5.focal_sets.end(); n++) {
                        if ((i->items & j->items & k->items & m->items & n->items).none()) {
                            // and add the combined mass to the conflict if the intersection is empty
                            conflict += i->mass * j->mass * k->mass * m->mass * n->mass;
                        }
                    }
                }
            }
...
double Evidence::conflictSix(Evidence& e1, Evidence& e2, Evidence& e3, Evidence& e4, Evidence& e5, Evidence& e6) {
    double conflict = 0.0;
    ...
    for (list<FocalSet>::iterator i = e1.focal_sets.begin(); i != e1.focal_sets.end(); i++) {
        for (list<FocalSet>::iterator j = e2.focal_sets.begin(); j != e2.focal_sets.end(); j++) {
            for (list<FocalSet>::iterator k = e3.focal_sets.begin(); k != e3.focal_sets.end(); k++) {
                for (list<FocalSet>::iterator m = e4.focal_sets.begin(); m != e4.focal_sets.end(); m++) {
                    for (list<FocalSet>::iterator n = e5.focal_sets.begin(); n != e5.focal_sets.end(); n++) {
                        for (list<FocalSet>::iterator p = e6.focal_sets.begin(); p != e6.focal_sets.end(); p++) {
                            if ((i->items & j->items & k->items & m->items & n->items & p->items).none()) {
                                // and add the combined mass to the conflict if the intersection is empty
                                conflict += i->mass * j->mass * k->mass * m->mass * n->mass * p->mass;
                            }
                        }
                    }
                }
...
double Evidence::conflictSeven(Evidence& e1, Evidence& e2, Evidence& e3, Evidence& e4, Evidence& e5, Evidence& e6, Evidence& e7) {
    double conflict = 0.0;
        ...
        for (list<FocalSet>::iterator j = e2.focal_sets.begin(); j != e2.focal_sets.end(); j++) {
            for (list<FocalSet>::iterator k = e3.focal_sets.begin(); k != e3.focal_sets.end(); k++) {
                for (list<FocalSet>::iterator m = e4.focal_sets.begin(); m != e4.focal_sets.end(); m++) {
                    for (list<FocalSet>::iterator n = e5.focal_sets.begin(); n != e5.focal_sets.end(); n++) {
                        for (list<FocalSet>::iterator p = e6.focal_sets.begin(); p != e6.focal_sets.end(); p++) {
                            for (list<FocalSet>::iterator q = e7.focal_sets.begin(); q != e7.focal_sets.end(); q++) {
                                if ((i->items & j->items & k->items & m->items & n->items & p->items & q->items).none()) {
                                    // and add the combined mass to the conflict if the intersection is empty
                                    conflict += i->mass * j->mass * k->mass * m->mass * n->mass * p->mass * q->mass;
                                }
                            }
                        }
                    }
